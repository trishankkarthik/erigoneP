...
procedure Trace is
  ...
  --   so some flags are needed to keep things in order
  procedure State_Loop is
    Acceptance:   Boolean := False; -- Start of acceptance cycle
    Printed_Next: Boolean;          -- Final state is not printed
                                    --   for example if assert if false
  begin
    ...
    loop
      ...
      -- If the simulation termination, there will be no next state line
      Printed_Next := False;
      exit when Index(S, "next state=") /= 1;
      -- There is a next state, so print the values of the previous one
      States.Print_State;
      Printed_Next := True;
    ...
    -- Print values even if no next state    
    if not Printed_Next then States.Print_State; end if;
    Put_Line(S(1..Length));
  end State_Loop;
