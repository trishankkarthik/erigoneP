...
with Global, Hash_Tables, Location_Stack, Options, State_Stack, Utilities;
package body Execute.Verify.Safety is
  procedure Search is
    Location:  Location_Stack.Location_Item; -- Current location
    Inserted:  Boolean;                      -- Was insert successful?
  ...
  exception
    when others =>
      Exception_Transition := Automata.Get_Transition(Location.L);
      raise;
  end Search;
