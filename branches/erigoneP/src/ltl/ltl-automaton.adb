...
with Ada.Text_IO;
with Automata.Display, Compile_Global, Config, Global, LTL.Formulas;
with Options, Utilities;
  ...
  procedure Convert(
    ...
    procedure Create_Transition(Position: in Node_Sets.Cursor) is
      ...
      begin
        ...
        -- Translate expression to byte code unless LTL_Only mode
        if Condition /= Blanks and
             Options.Execution_Mode /= Options.LTL_Only then
          declare
            S: String := Compile_Global.Compile_Expression(Condition);
          begin
            Automata.Extract_Byte_Code(
              S(2..S'Length-2), Code_Size, Byte_Code);
          end;
        end if;
        ...
        LTL_Transitions(Count) :=
          (Source      => Source,
          Target       => N.N,
          Statement    => new String'(Condition),
          End_Label    => End_Label,
    ...
    procedure Optimize is
      ...
      begin  -- Replace_State
        ...
        -- For debugging optimization step by step:
        ...
        for Index1 in 0..Count-2 loop
          ...
          for Index2 in Index1+1 .. Count-1 loop
            ...
            if T(Index1).Source      /= T(Index2).Source and
               ...
               T(Index1).Statement    = T(Index2).Statement and
              ...
              while Index2+N <= Count-1 loop
                ...
                exit Second_Source_Loop when
                  ...
                  T(Index1+N).Statement    /= T(Index2+N).Statement or
