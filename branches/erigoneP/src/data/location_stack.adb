...
package body Location_Stack is
  ...
  Location_Stack: access Location_Array; -- The stack is allocated at runtime
  ...
  procedure Put_Trail(
      Error_Count: in Positive; Start_Of_Acceptance_Cycle: in Integer) is
  ...
  begin
    ...
    if Options.Error_Number = 0 then
      Files.Trail_File_Name :=
        new String'(Files.Root_File_Name.all &
          Utilities.Trim(Integer'Image(Error_Count)) & Files.Trail_Extension);
    end if;
    ...
    for I in 0 .. Top1 loop
      if Location_Stack(I).Visited then
