...
package body Compress_Vectors.Hash_Tables is
  ...
  procedure Put_Hash is
    ...
    use Utilities;
  begin
    Put("states stored=", Stored_Elements);
    Put("matched=",       Matched_Elements);
    Put("total=",         Stored_Elements + Matched_Elements);
    Put("collisions=",    Collisions, New_Line => True);

    Put("element size=",  Integer'(S/Byte'Size));
    Put("memory=",        Stored_Elements*S/Byte'Size);
    Put("overhead=",      Positive(Hash_Slots) * (Hash_Access'Size/Byte'Size) +
